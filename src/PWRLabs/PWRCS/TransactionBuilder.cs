...
public class TransactionBuilder {
  ...
  /// <param name="nonce">The nonce value of the transaction.</param>
  /// <returns>The constructed transaction.</returns>
  public async Task<byte[]> GetJointxn(string ip, uint nonce, byte chainId) {
  ...
  public async Task<byte[]> GetClaimActiveNodeSpotTxn(uint nonce,
                                                      byte chaindId) {
    byte[] txnBase = await GetTxnBase((byte) 2, nonce, chaindId);
    return txnBase;
  }
  /// <summary>
  ...
  /// <summary>
  /// Constructs a transaction for withdrawing PWR tokens from the specified
  /// address.
  /// </summary>
  /// <param name="from">The sender's address.</param>
  ...
  /// <param name="nonce">The nonce value of the transaction.</param>
  /// <returns>The constructed transaction.</returns>
  public async Task<byte[]> GetWithdrawPWRTxn(string validator,
                                              ulong sharesAmount, uint nonce,
                                              byte chainId) {
    ValidateAddress(validator);

    byte[] txnBase = await GetTxnBase(4, nonce, chainId);
    byte[] fromBytes = Extensions.HexStringToByteArray(validator);
  ...
  public async Task<byte[]> GetVmDataTxn(ulong vmId, byte[] data, uint nonce,
    ...
    using(MemoryStream stream =
              new MemoryStream(txnBase.Length + 8 + data.Length)) {
      stream.Write(txnBase, 0, txnBase.Length);
  ...
  /// <param name="nonce">The nonce value of the transaction.</param>
  /// <returns>The constructed transaction.</returns>
  public async Task<byte[]> GetSendConduitTransactionTxn(ulong vmId, byte[] txn,
                                                         uint nonce,
                                                         byte chainId) {
    byte[] txnBase = await GetTxnBase(11, nonce, chainId);

    using(MemoryStream stream =
              new MemoryStream(txnBase.Length + 8 + txn.Length)) {
      stream.Write(txnBase, 0, txnBase.Length);
      byte[] vmIdBytes = BitConverter.GetBytes(vmId);
      Array.Reverse(vmIdBytes);
  ...
  public async Task<byte[]> GetSetGuardianTxn(string guardianAddress,
    ...
    byte[] guardianAddressBytes =
        Extensions.HexStringToByteArray(guardianAddress);
  ...
  public async Task<byte[]> GetPayableVmDataTxn(ulong vmId, ulong value,
    ...
    byte[] txnBase = await GetTxnBase(5, nonce, chainId);
    using MemoryStream stream =
        new MemoryStream(txnBase.Length + 16 + data.Length);
    stream.Write(txnBase);
    stream.Write(GetBigEndianBytes(vmId));
    stream.Write(data);
  ...
  private byte[] GetBigEndianBytes(ulong value) {
